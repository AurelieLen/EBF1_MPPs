...
def assign_sex(data, ygenes, use_raw = True):
    ...
    data.obs.loc[(data.obs.xist_bin == True) & (data.obs.Ygene_bin == False), data.obs.columns == 'sex'] = 'female'
    data.obs.loc[(data.obs.xist_bin == True) & (data.obs.Ygene_bin == True), data.obs.columns == 'sex'] = 'mfdoublet'
    data.obs.loc[(data.obs.xist_bin == False) & (data.obs.Ygene_bin == True), data.obs.columns == 'sex'] = 'male'
...
def process(adata,
    ...
    pca, nearest neighbour selection, clustering, umap calculation
    ...
    if regress_cc:
        adata.X = logn_bak.X.copy() ##it does not look like there is a big difference whether the regression is done on scale or lognn data, at least on a small scale
    ...
    if batch_correct  == 'bbknn':
        sc.external.pp.bbknn(adata, n_pcs = n_pcs, neighbors_within_batch = 5)
...
def cmap_RdBu2(values, vmin = None, vmax = None):
    """Generates a blue/red colorscale with white value centered around the value 0, needs both negative and positive values.
...
from scipy.sparse import issparse
##### Fixed gene scoring functions froms scnapy, improved precision
