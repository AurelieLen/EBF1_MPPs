...
```{r WT maplot, message=FALSE}
WT_M3vM4_df <- as.data.frame(resLFC_M3WTvM4WT)
WT_M3vM4_df$padj[is.na(WT_M3vM4_df$padj)] <- 1
WT_M3vM4_df$sigBool <- 'NS'
WT_M3vM4_df[(WT_M3vM4_df$padj <0.1) & (WT_M3vM4_df$log2FoldChange > 0),]$sigBool <- "MPP3"
WT_M3vM4_df[(WT_M3vM4_df$padj <0.1) & (WT_M3vM4_df$log2FoldChange < 0),]$sigBool <- "MPP4"
...
WT_MPP3vMPP4_MA <- ggplot(data=WT_M3vM4_df) +
  ...
  scale_color_manual(values = c("#e87b2a","#1272ba","grey" ))
...
```
```{r export MPP3, MPP4 and shared DE gene list}
MPP4_Up <- M4_spec[M4_spec$log2FoldChange > 0,]
MPP4_Down <- M4_spec[M4_spec$log2FoldChange < 0,]
MPP3_Up <- M3_spec[M3_spec$log2FoldChange > 0,]
MPP3_Down <- M3_spec[M3_spec$log2FoldChange < 0,]
Common_Up <- df_shared[df_shared$log2FoldChange > 1,]
Common_Down <- df_shared[df_shared$log2FoldChange < 1,]
write.table(MPP4_Up$gene, file = "../proc_data/MPP4_Up_genename.txt", quote = FALSE, row.names = FALSE, col.names = FALSE)
write.table(MPP4_Down$gene, file = "../proc_data/MPP4_Down_genename.txt", quote = FALSE, row.names = FALSE, col.names = FALSE)
write.table(MPP3_Up$gene, file = "../proc_data/MPP3_Up_genename.txt", quote = FALSE, row.names = FALSE, col.names = FALSE)
write.table(MPP3_Down$gene, file = "../proc_data/MPP3_Down_genename.txt", quote = FALSE, row.names = FALSE, col.names = FALSE)
write.table(Common_Up$gene, file = "../proc_data/Common_Up_genename.txt", quote = FALSE, row.names = FALSE, col.names = FALSE)
write.table(Common_Down$gene, file = "../proc_data/Common_Down_genename.txt", quote = FALSE, row.names = FALSE, col.names = FALSE)
```
...
rownames(M4_spec) = M4_spec$Row.names
rownames(df_shared) <- df_shared$Row.names
M3_spec = M3_spec[,c(1:15)]
M4_spec = M4_spec[,c(1:7,16:23)]
```

Plot heatmap for MPP3 DE genes. 
Calculate z-scores for each set of DE genes.
```{r M3 DRG zscore}
M3spec_Zscores <- as.data.frame(
  t(apply(M3_spec[,c(8:15)], 1, function(x) (x-mean(x))/sd(x)) ))
M3spec_Zscores <- M3spec_Zscores[,c(5,6,7,8,1,2,3,4)]

M3shared_Zscores <- as.data.frame(t(apply(df_shared[,c(8:15)], 1, function(x) (x-mean(x))/sd(x)) ))
M3shared_Zscores <- M3shared_Zscores[,c(5,6,7,8,1,2,3,4)]
```

Extract cluster row order for each set of DE genes. 
```{r M3 DRG roworder}
pal <- wes_palette(name = "Zissou1", 100, type = c("continuous"))
M3_spec_heatmap <- pheatmap(as.matrix(M3spec_Zscores), cluster_cols=F, cellwidth = 8, cellheight = 2, treeheight_row = 0, show_rownames = FALSE, color = pal, border_color = "black")
M3_spec_heatmap_roworder <- M3spec_Zscores[M3_spec_heatmap$tree_row$order,]
rev_M3_spec_heatmap_roworder <- apply(M3_spec_heatmap_roworder, 2, rev)
rev_M3_spec_heatmap_roworder <- as.data.frame(rev_M3_spec_heatmap_roworder)
M3_roworder_anno <- data.frame(gene = row.names(rev_M3_spec_heatmap_roworder))
M3_roworder_anno$cluster <- "MPP3"
...
M3_roworder_anno$gene <- NULL
```
```{r common MPP3 DRG roworder}
shared_MPP3 <- pheatmap(as.matrix(M3shared_Zscores), cluster_cols=F, cellwidth = 8, cellheight = 2, treeheight_row = 0, legend_breaks = c(-2, 0, 2), show_rownames = FALSE, color = pal)
sharedM3_heatmap_roworder <- M3shared_Zscores[shared_MPP3$tree_row$order,]
commonM3_roworder_anno <- data.frame(gene = row.names(sharedM3_heatmap_roworder))
commonM3_roworder_anno$cluster <- "common"
commonM3_roworder_anno$gene -> row.names(commonM3_roworder_anno)
commonM3_roworder_anno$gene <- NULL
```

Plot heatmap or ordered DRGs.
```{r MPP3 DRG heatmap}
M3ordered_DRG_mat <- rbind(rev_M3_spec_heatmap_roworder,sharedM3_heatmap_roworder)
M3ordered_DRG_anno <- rbind(M3_roworder_anno, commonM3_roworder_anno)

pheatmap(as.matrix(M3ordered_DRG_mat), cluster_cols=F, cluster_rows = F, cellwidth = 6, cellheight = 1,
         treeheight_row = 0, annotation_row = M3ordered_DRG_anno, show_rownames = FALSE, color = pal,
         gaps_row = c(149), filename = "../figures/MPP3_DRG_zscore_heatmap.pdf")
```


Plot heatmap for MPP4 DE genes. 
Calculate z-scores for each set of DE genes.
```{r M4 DRG zscore}
M4spec_Zscores <- as.data.frame(
  t(apply(M4_spec[,c(8:15)], 1, function(x) (x-mean(x))/sd(x)) ))
M4spec_Zscores <- M4spec_Zscores[,c(5,6,7,8,1,2,3,4)]

M4shared_Zscores <- as.data.frame(t(apply(df_shared[,c(16:23)], 1, function(x) (x-mean(x))/sd(x)) ))
M4shared_Zscores <- M4shared_Zscores[,c(5,6,7,8,1,2,3,4)]
```

Extract cluster row order for each set of DE genes. 
```{r M4 DRG roworder}
M4_spec_heatmap <- pheatmap(as.matrix(M4spec_Zscores), cluster_cols=F, cellwidth = 8, cellheight = 2, treeheight_row = 0, show_rownames = FALSE, color = pal, border_color = "black")
M4_spec_heatmap_roworder <- M4spec_Zscores[M4_spec_heatmap$tree_row$order,]
rev_M4_spec_heatmap_roworder <- apply(M4_spec_heatmap_roworder, 2, rev)
rev_M4_spec_heatmap_roworder <- as.data.frame(rev_M4_spec_heatmap_roworder)
M4_roworder_anno <- data.frame(gene = row.names(rev_M4_spec_heatmap_roworder))
M4_roworder_anno$cluster <- "MPP4"
...
M4_roworder_anno$gene <- NULL
```
```{r common MPP4 DRG roworder}
shared_MPP4 <- pheatmap(as.matrix(M4shared_Zscores), cluster_cols=F, cellwidth = 8, cellheight = 2, treeheight_row = 0, legend_breaks = c(-2, 0, 2), show_rownames = FALSE, color = pal)
sharedM4_heatmap_roworder <- M4shared_Zscores[shared_MPP4$tree_row$order,]
rev_sharedM4_heatmap_roworder <- apply(sharedM4_heatmap_roworder, 2, rev)
commonM4_roworder_anno <- data.frame(gene = row.names(rev_sharedM4_heatmap_roworder))
commonM4_roworder_anno$cluster <- "common"
commonM4_roworder_anno$gene -> row.names(commonM4_roworder_anno)
commonM4_roworder_anno$gene <- NULL
```
Plot heatmap or ordered DRGs.
```{r MPP4 DRG heatmap}
M4ordered_DRG_mat <- rbind(rev_M4_spec_heatmap_roworder,rev_sharedM4_heatmap_roworder)
M4ordered_DRG_anno <- rbind(M4_roworder_anno, commonM4_roworder_anno)

pheatmap(as.matrix(M4ordered_DRG_mat), cluster_cols=F, cluster_rows = F, cellwidth = 6, cellheight = 1,
         treeheight_row = 0, annotation_row = M4ordered_DRG_anno, show_rownames = FALSE, color = pal,
         gaps_row = c(239), filename = "../figures/MPP4_DRG_zscore_heatmap.pdf")
```
...
pheatmap(as.matrix(df_cebpa_GO_zscore), cluster_cols=F, cluster_rows = F, cellwidth = 6, cellheight = 6,
         treeheight_row = 0, annotation_row = df_cebpa_GO_anno, show_rownames = T,
         color = pal, gaps_row = c(36, 53), gaps_col = c(8), 
         filename = "../figures/GO_cebpa_down_MPP3_MPP4.pdf")
```
...
MPP3 DE genes
```{r MPP3 DRG cebpa targets}
M3ordered_DRG_mat_cebpa <- M3ordered_DRG_mat
M3ordered_DRG_mat_cebpa$gene <- row.names(M3ordered_DRG_mat_cebpa)
M3DRG_cebpa<-left_join(x=M3ordered_DRG_mat_cebpa,y=cebpa_peaks_anno_dedup,by="gene")
M3DRG_cebpa[is.na(M3DRG_cebpa)] <- 0
M3DRG_cebpa$chr[M3DRG_cebpa$chr > 0] <- 1
identical(M3DRG_cebpa$gene, M3ordered_DRG_mat_cebpa$gene)
M3DRG_cebpa$gene <- factor(M3DRG_cebpa$gene, levels = as.character(M3DRG_cebpa$gene))
MPP3_DRG_cebpa_peak <- ggplot(M3DRG_cebpa, aes(x = 1, y = gene)) + 
  geom_tile(aes(fill=chr)) +
  scale_y_discrete(limits=rev) + scale_fill_manual(values = c("ivory2", "royalblue3")) +
  theme_minimal() + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
ggsave("../figures/MPP3_DRG_zscore_heatmap_cebpa_peak.svg", dpi=300, width = 4, units = "in")
```

MPP4 DE genes
```{r MPP4 DRG cebpa targets}
M4ordered_DRG_mat_cebpa <- M4ordered_DRG_mat
M4ordered_DRG_mat_cebpa$gene <- row.names(M4ordered_DRG_mat_cebpa)
M4DRG_cebpa<-left_join(x=M4ordered_DRG_mat_cebpa,y=cebpa_peaks_anno_dedup,by="gene")
M4DRG_cebpa[is.na(M4DRG_cebpa)] <- 0
M4DRG_cebpa$chr[M4DRG_cebpa$chr > 0] <- 1
identical(M4DRG_cebpa$gene, M4ordered_DRG_mat_cebpa$gene)
M4DRG_cebpa$gene <- factor(M4DRG_cebpa$gene, levels = as.character(M4DRG_cebpa$gene))
MPP4_DRG_cebpa_peak <- ggplot(M4DRG_cebpa, aes(x = 1, y = gene)) + 
  geom_tile(aes(fill=chr)) +
  scale_y_discrete(limits=rev) + scale_fill_manual(values = c("ivory2", "royalblue3")) +
  theme_minimal() + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
ggsave("../figures/MPP4_DRG_zscore_heatmap_cebpa_peak.svg", dpi=300, width = 4, units = "in")
```
